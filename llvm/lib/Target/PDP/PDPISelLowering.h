//===-- PDPISelLowering.h - PDP DAG Lowering Interface ----------*- C++ -*-===//
//
...
#ifndef LLVM_PDP_ISEL_LOWERING_H
#define LLVM_PDP_ISEL_LOWERING_H
...
enum NodeType {
  ...
  /// PDP conditional branches. Operand 0 is the chain operand, operand 1
...
} // end of namespace PDPISD

class PDPSubtarget;
class PDPTargetMachine;

/// Performs target lowering for the PDP.
class PDPTargetLowering : public TargetLowering {
public:
  explicit PDPTargetLowering(const PDPTargetMachine &TM,
...
public:
  ...
  MachineBasicBlock *
  EmitSelect(MachineInstr &MI, MachineBasicBlock *MBB) const;

  MachineBasicBlock *
  EmitSelect2(MachineInstr &MI, MachineBasicBlock *MBB) const;
  ...
  splitValueIntoRegisterParts(SelectionDAG &DAG, const SDLoc &DL, SDValue Val,
                              SDValue *Parts, unsigned int NumParts, MVT PartVT,
  ...
  joinRegisterPartsIntoValue(SelectionDAG &DAG, const SDLoc &DL,
                             const SDValue *Parts, unsigned int NumParts,
                             MVT PartVT, EVT ValueVT,
                             std::optional<CallingConv::ID> CC) const override;

private:
  SDValue getPDPCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC, SDValue &PDPcc,
                    SelectionDAG &DAG, SDLoc dl) const;
  SDValue getPDPCmp(SDValue LHS, SDValue RHS, SelectionDAG &DAG,
                    SDLoc dl) const;
  ...
  SDValue LowerBR_CC2(SDValue Op, SelectionDAG &DAG) const;

  SDValue LowerINLINEASM(SDValue Op, SelectionDAG &DAG) const;

  SDValue LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const;
  SDValue LowerSELECT_CC2(SDValue Op, SelectionDAG &DAG) const;

  SDValue LowerSETCC(SDValue Op, SelectionDAG &DAG) const;
  SDValue LowerSETCC2(SDValue Op, SelectionDAG &DAG) const;

  SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG) const;
  SDValue LowerLOAD(SDValue Op, SelectionDAG &DAG) const;
  SDValue LowerLOAD(SDNode *N, SmallVectorImpl<SDValue> &Results, SelectionDAG &DAG) const;
...
private:
  ...
  //MachineBasicBlock *insertCopyZero(MachineInstr &MI, MachineBasicBlock *BB) const;
...
#endif // LLVM_PDP_ISEL_LOWERING_H
