...
void Z80AsmBackend::adjustFixupValue(const MCFixup &Fixup,
  ...
  uint64_t Size = Z80AsmBackend::getFixupKindInfo(Fixup.getKind()).TargetSize;
  ...
  case Z80::fixup_hh8_ldi_pm:
    if (Kind == Z80::fixup_hh8_ldi_pm) adjust::pm(Value);
  ...
  case Z80::fixup_lo8_ldi_pm_neg:
    if (Kind == Z80::fixup_lo8_ldi_pm_neg) adjust::pm(Value);
  ...
  case Z80::fixup_hi8_ldi_pm_neg:
    if (Kind == Z80::fixup_hi8_ldi_pm_neg) adjust::pm(Value);
  ...
  case Z80::fixup_hh8_ldi_pm_neg:
    if (Kind == Z80::fixup_hh8_ldi_pm_neg) adjust::pm(Value);
