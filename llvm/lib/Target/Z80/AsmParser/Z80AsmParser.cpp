...
namespace {
/// Parses Z80 assembly from a stream.
...
#include "Z80GenAsmMatcher.inc"
  ...
  unsigned toDREG(unsigned Reg, unsigned From = Z80::sub_lo) {
    MCRegisterClass const *Class = &Z80MCRegisterClasses[Z80::DREGSRegClassID];
...
public:
  ...
  static std::unique_ptr<Z80Operand> CreateToken(StringRef Str, SMLoc S) {
    return std::make_unique<Z80Operand>(Str, S);
  }

  static std::unique_ptr<Z80Operand> CreateReg(unsigned RegNum, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<Z80Operand>(RegNum, S, E);
  }

  static std::unique_ptr<Z80Operand> CreateImm(const MCExpr *Val, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<Z80Operand>(Val, S, E);
  }

  static std::unique_ptr<Z80Operand>
  CreateMemri(unsigned RegNum, const MCExpr *Val, SMLoc S, SMLoc E) {
    return std::make_unique<Z80Operand>(RegNum, Val, S, E);
  }
...
bool Z80AsmParser::invalidOperand(SMLoc const &Loc,
  ...
  if (ErrorInfo != ~0U) {
    if (ErrorInfo >= Operands.size()) {
      ...
      Z80Operand const &Op = (Z80Operand const &)*Operands[ErrorInfo];
...
bool Z80AsmParser::missingFeature(llvm::SMLoc const &Loc,
                                  uint64_t const &ErrorInfo) {
...
bool Z80AsmParser::MatchAndEmitInstruction(SMLoc Loc, unsigned &Opcode,
...
int Z80AsmParser::parseRegisterName(unsigned (*matchFn)(StringRef)) {
  StringRef Name = Parser.getTok().getString();

  int RegNum = matchFn(Name);

  // GCC supports case insensitive register names. Some of the Z80 registers
...
int Z80AsmParser::parseRegisterName() {
...
int Z80AsmParser::parseRegister(bool RestoreOnFailure) {
  int RegNum = Z80::NoRegister;
  ...
  if (Parser.getTok().is(AsmToken::Identifier)) {
    ...
    if (Parser.getLexer().peekTok().is(AsmToken::Colon)) {
      ...
      if (RegNum == Z80::NoRegister && RestoreOnFailure) {
...
bool Z80AsmParser::tryParseRegisterOperand(OperandVector &Operands) {
  ...
  if (RegNo == Z80::NoRegister)
    return true;

  AsmToken const &T = Parser.getTok();
  Operands.push_back(Z80Operand::CreateReg(RegNo, T.getLoc(), T.getEndLoc()));
  Parser.Lex(); // Eat register token.

  return false;
}

bool Z80AsmParser::tryParseExpression(OperandVector &Operands) {
  ...
  Operands.push_back(Z80Operand::CreateImm(Expression, S, E));
  return false;
}

bool Z80AsmParser::tryParseRelocExpression(OperandVector &Operands) {
  llvm_unreachable("Z80AsmParser::tryParseRelocExpression");
/*  bool isNegated = false;
  ...
  ModifierKind = Z80MCExpr::getKindByName(ModifierName.str().c_str());
  ...
  if (ModifierKind != Z80MCExpr::VK_Z80_None) {
    ...
    if (Parser.getTok().getString() == GENERATE_STUBS &&
      ...
      ModifierKind = Z80MCExpr::getKindByName(GSModName.c_str());
      if (ModifierKind != Z80MCExpr::VK_Z80_None)
  ...
  MCExpr const *Expression = Z80MCExpr::create(ModifierKind, InnerExpression,
...
bool Z80AsmParser::parseOperand(OperandVector &Operands) {
  ...
  case AsmToken::Minus: {
    ...
    Operands.push_back(Z80Operand::CreateToken(Parser.getTok().getString(),
...
Z80AsmParser::parseMemriOperand(OperandVector &Operands) {
  ...
  Operands.push_back(Z80Operand::CreateMemri(RegNo, Expression, S, E));
...
bool Z80AsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
  ...
  return (RegNo == Z80::NoRegister);
}

OperandMatchResultTy Z80AsmParser::tryParseRegister(unsigned &RegNo,
                                                    SMLoc &StartLoc,
                                                    SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
...
bool Z80AsmParser::ParseDirective(llvm::AsmToken DirectiveID) {
...
bool Z80AsmParser::parseLiteralValues(unsigned SizeInBytes, SMLoc L) {
  llvm_unreachable("Z80AsmParser::parseLiteralValues");
  /*MCAsmParser &Parser = getParser();
  Z80MCELFStreamer &Z80Streamer =
      static_cast<Z80MCELFStreamer &>(Parser.getStreamer());
  AsmToken Tokens[2];
  size_t ReadCount = Parser.getLexer().peekTokens(Tokens);
  if (ReadCount == 2 && Parser.getTok().getKind() == AsmToken::Identifier &&
    ...
    Z80Streamer.emitValueForModiferKind(Symbol, SizeInBytes, L,
            Z80MCExpr::VK_Z80_None);
    return false;
  }
  ...
  if (Parser.getTok().getKind() == AsmToken::Identifier &&
    ...
    Z80MCExpr::VariantKind ModifierKind =
        Z80MCExpr::getKindByName(ModifierName.str().c_str());
    if (ModifierKind != Z80MCExpr::VK_Z80_None) {
      Parser.Lex();
    ...
    Z80Streamer.emitValueForModiferKind(Symbol, SizeInBytes, L, ModifierKind);
    return false;
  }
...
unsigned Z80AsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
  ...
  if (Op.isReg()) {
    ...
    if (isSubclass(Expected, MCK_DREGS)) {
      ...
      if (correspondingDREG != Z80::NoRegister) {
