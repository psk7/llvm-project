...
//===----------------------------------------------------------------------===//
//
// PDP11 is a Harvard-architecture 8-bit microcontroller designed for small
// baremetal programs. All PDP11-family processors have 32 8-bit registers.
// The tiniest PDP11 has 32 byte RAM and 1 KiB program memory, and the largest
...
// ELF files on PDP11s. You are expected to link your program against address
...
//
// Note that the current PDP11 support is very preliminary so you can't
...
namespace {
class PDP11 final : public TargetInfo {
public:
  PDP11(Ctx &ctx) : TargetInfo(ctx) { needsThunks = true; }
...
RelExpr PDP11::getRelExpr(RelType type, const Symbol &s,
                        const uint8_t *loc) const {
  switch (type) {
  case R_PDP11_16_0:
    return R_ABS;
  case R_PDP11_6_PCREL:
  case R_PDP11_8_PCREL:
  case R_PDP11_16_PCREL:
    return R_PC;
  default:
...
void PDP11::relocate(uint8_t *loc, const Relocation &rel, uint64_t val) const {
  switch (rel.type) {
    case R_PDP11_16_0:
  ...
  case R_PDP11_LO8_LDI_NEG:
    writeLDI(loc, -val & 0xff);
    break;
  case R_PDP11_LO8_LDI:
    writeLDI(loc, val & 0xff);
    break;
  case R_PDP11_HI8_LDI_NEG:
    writeLDI(loc, (-val >> 8) & 0xff);
    break;
  case R_PDP11_HI8_LDI:
    writeLDI(loc, (val >> 8) & 0xff);
    break;
  case R_PDP11_HH8_LDI_NEG:
    writeLDI(loc, (-val >> 16) & 0xff);
    break;
  case R_PDP11_HH8_LDI:
    writeLDI(loc, (val >> 16) & 0xff);
    break;
  case R_PDP11_MS8_LDI_NEG:
    writeLDI(loc, (-val >> 24) & 0xff);
    break;
  case R_PDP11_MS8_LDI:
    writeLDI(loc, (val >> 24) & 0xff);
    break;

  case R_PDP11_LO8_LDI_GS:
    checkUInt(ctx, loc, val, 17, rel);
    [[fallthrough]];
  case R_PDP11_LO8_LDI_PM:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (val >> 1) & 0xff);
    break;
  case R_PDP11_HI8_LDI_GS:
    checkUInt(ctx, loc, val, 17, rel);
    [[fallthrough]];
  case R_PDP11_HI8_LDI_PM:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (val >> 9) & 0xff);
    break;
  case R_PDP11_HH8_LDI_PM:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (val >> 17) & 0xff);
    break;

  case R_PDP11_LO8_LDI_PM_NEG:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (-val >> 1) & 0xff);
    break;
  case R_PDP11_HI8_LDI_PM_NEG:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (-val >> 9) & 0xff);
    break;
  case R_PDP11_HH8_LDI_PM_NEG:
    checkAlignment(ctx, loc, val, 2, rel);
    writeLDI(loc, (-val >> 17) & 0xff);
    break;

  case R_PDP11_LDS_STS_16: {
...
void elf::setPDP11TargetInfo(Ctx &ctx) { ctx.target.reset(new PDP11(ctx)); }

static uint32_t getEFlags(InputFile *file) {
  return cast<ObjFile<ELF32LE>>(file)->getObj().getHeader().e_flags;
}

uint32_t PDP11::calcEFlags() const {
  assert(!ctx.objectFiles.empty());

  uint32_t flags = getEFlags(ctx.objectFiles[0]);
  bool hasLinkRelaxFlag = flags & EF_PDP11_LINKRELAX_PREPARED;

  for (InputFile *f : ArrayRef(ctx.objectFiles).slice(1)) {
    uint32_t objFlags = getEFlags(f);
    if ((objFlags & EF_PDP11_ARCH_MASK) != (flags & EF_PDP11_ARCH_MASK))
      ErrAlways(ctx)
          << f << ": cannot link object files with incompatible target ISA";
    if (!(objFlags & EF_PDP11_LINKRELAX_PREPARED))
      hasLinkRelaxFlag = false;
  }

  if (!hasLinkRelaxFlag)
    flags &= ~EF_PDP11_LINKRELAX_PREPARED;
